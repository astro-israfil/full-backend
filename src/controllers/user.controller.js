...
const getUserChannelDetails = asyncHandler(async function (req, res) {
  const { username } = req.params;
  if (!username?.trim()) {
    throw new ApiError(400, "Username is not in request");
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { channel: channel[0] }, "success"));
});
